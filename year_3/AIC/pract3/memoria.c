...
void inicializa_memoria(
    ...
    for (dword i = 0; i < TAM_MEMO_INSTRUC / 4; i++) {
        mm->instruc[i].codop = RV32I_ADDI;
        mm->instruc[i].tipo = FormatoI;
        mm->instruc[i].rd = 0;
        mm->instruc[i].rs1 = 0;
        mm->instruc[i].rs2 = 0;
        mm->instruc[i].imm = 0;
        mm->instruc[i].valor = 0x00000013;
    } /* endfor */
...
valor_t lee_mem_datos(
    ...
    if (es_region_datos(r)) {
        ...
        fprintf(stderr, "ERROR: Lectura fuera del rango de memoria de datos (PC: " F_DWORD ", dir: " F_DWORD ")\n",
                PC,direccion);
        exit(1);
    }
    return resultado;
...
/**
 * Comprueba si la dirección de memoria es un dirección valida de la memoria de datos
 *
 * @param mm Mapa de memoria.
 * @param direccion Dirección absoluta del dato
 * @return boolean
...
void escribe_mem_datos(
    ...
    if (es_region_datos(r)) {
        ...
        fprintf(stderr, "ERROR: Escritura fuera del rango de memoria de datos (PC: " F_DWORD ", dir: " F_DWORD ")\n", PC, direccion);
        exit(1);
    }
