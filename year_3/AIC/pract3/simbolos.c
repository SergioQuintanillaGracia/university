...
static
ref_direccion_t *nueva_direccion(
        char *nombre,
        int indice,
        dword offset,
        tipo_direccion_t tipo_dir
) {
    ref_direccion_t *referencia;

    referencia = (ref_direccion_t *) malloc(sizeof(ref_direccion_t));

    strncpy(referencia->texto, nombre, LONG_ETIQUETA);
    referencia->texto[LONG_ETIQUETA - 1] = '\0';
    referencia->indice = indice;
    referencia->offset = offset;
    referencia->tipo_dir = tipo_dir;
    referencia->sig = NULL;
...
/**
 * Calcula el desplazamiento para una instrucción.
 *
 */
static dword calcula_inmediato_referencia(
        char *fichero,
        referencia_t *p_ref,
        dword dir_ref,
        dword dir_instruc,
    ...
    switch (p_ref->parte_dir) {
        case PC_REL_LO_12_SHIFT:
            // Este offset relativo es el de los saltos que está dividido por 2
            offset = dir_ref - dir_instruc;
            offset >>= 1;
            if (offset > MAX_IMM || offset < MIN_IMM) {
                fprintf(stderr,
                        "%s: Valor de desplazamiento para la referencia '%s' (" F_DWORD ") fuera de límite.",
                        fichero, p_ref->texto, offset);
                exit(1);
            }
            valor = offset * 2;
            offset &= 0xFFF;

            DEBUG("PCREL_LO_12_SHIFT: Referencia '%s'. PC: " F_DWORD " Dir: " F_DWORD,
                  p_ref->texto, dir_instruc, dir_ref);
            DEBUG("%s: Referencia '%s' resuelta: PC-REL-LO-12-SHIFT (PC: " F_DWORD ") -> " F_DWORD,
                  fichero, p_ref->texto, p_ref->offset, valor);

            break;
        case PC_REL_LO_20_SHIFT:
            // Este offset relativo es el de los saltos que está dividido por 2
            offset = dir_ref - dir_instruc;
            offset >>= 1;
            if (offset > MAX_IMM_20 || offset < MIN_IMM_20) {
                fprintf(stderr,
                        "%s: Valor de desplazamiento para la referencia '%s' (" F_DWORD ") fuera de límite.",
                        fichero, p_ref->texto, offset);
                exit(1);
            ...
            DEBUG("PCREL_LO_20_SHIFT: Referencia '%s'. PC: " F_DWORD " Dir: " F_DWORD,
                  p_ref->texto, dir_instruc, dir_ref);
            DEBUG("%s: Referencia '%s' resuelta: PC-REL-LO-20-SHIFT (PC: " F_DWORD ") -> " F_DWORD,
                  fichero, p_ref->texto, p_ref->offset, valor);

            break;
        case PC_REL_LO_12:
            // TODO: Eliminar este + 4 modificando input.yacc.y
            // Se añade + 4 porque la segunda instrucción que se ha generado a partir de una pseudo
            //  ya ha incrementado el PC y, por lo tanto, la distancia con la referencia
            //  se estaría calculando mal.
            offset = dir_ref - dir_instruc + 4;
            offset &= 0xFFF;

            DEBUG("PCREL_LO_12: Referencia '%s'. PC: " F_DWORD " Dir: " F_DWORD,
                  p_ref->texto, dir_instruc, dir_ref);
            DEBUG("%s: Referencia '%s' resuelta: PC-REL-LO-12 (PC: " F_DWORD ") -> " F_DWORD,
                  fichero, p_ref->texto, p_ref->offset, offset);

            break;
        case PC_REL_HI_20:
            offset = dir_ref - dir_instruc;
            // Si el desplazamiento de 12 bits va a ser negativo hay que sumarle 1 a HI20
            offset11 = (offset & 0x800) >> 11;
            offset >>= 12;
            offset += offset11;
            offset &= 0xFFFFF;

            DEBUG("PCREL_HI_20: Referencia '%s'. PC: " F_DWORD " Dir: " F_DWORD,
                  p_ref->texto, dir_instruc, dir_ref);
            DEBUG("%s: Referencia '%s' resuelta: PC-REL-HI-20 (PC: " F_DWORD ") -> " F_DWORD,
                  fichero, p_ref->texto, p_ref->offset, offset);
        ...
        case ABS_LO_12:
            ...
            DEBUG("%s: Referencia '%s' resuelta: %%lo(A32)", fichero, p_ref->texto);
        ...
        case ABS_HI_20:
            ...
            DEBUG("%s: Referencia '%s' resuelta: %%hi(A32)", fichero, p_ref->texto);

            break;
        case DATA_REL_LO_12:
            offset = dir_ref - inicio_datos;
            // Risc-V custom DISP12
            if (offset > MAX_IMM) {
                fprintf(stderr, "Error. Desplazamiento para la referencia '%s' es mayor que %d (%ld).\n",
                        p_ref->texto, (int)MAX_IMM, offset);
                fprintf(stderr, "       Instrucción con offset: %ld\n", p_ref->offset);
                fprintf(stderr, "       Se sugiere utilizar la pseudo-instrucción 'la'.\n");
                exit(1);
            }
            offset &= 0xFFF;

            DEBUG("%s: Referencia '%s' resuelta: %%disp(A32)", fichero, p_ref->texto);

            break;
        default:
            fprintf(stderr, "%s: Referencia '%s'. Tipo no soportado.", fichero, p_ref->texto);
            exit(1);
            break;
    }
...
static void resuelve_direccion_referencia(
        char *fichero,
        ref_direccion_t *p_dir,
        dword dir_ref
) {
    dword direccion;

    direccion = dir_ref + p_dir->offset;

    switch (p_dir->tipo_dir) {
        case DIR_REG:
            escribe_int_reg(p_dir->indice, direccion, t_dword, NO);

            DEBUG("%s: Dirección '%s' resuelta: Rint[%d] = " F_DWORD,
                  fichero, p_dir->texto, p_dir->indice, direccion);

            break;
        case DIR_BTB_PC:
            BTB[p_dir->indice].PC = direccion;

            DEBUG("%s: Dirección '%s' resuelta: BTB[%d].PC = " F_DWORD,
                  fichero, p_dir->texto, p_dir->indice, direccion);

            break;
        case DIR_BTB_DEST:
            BTB[p_dir->indice].destino = direccion;
            DEBUG("%s: Dirección '%s' resuelta: BTB[%d].destino = " F_DWORD,
                  fichero, p_dir->texto, p_dir->indice, direccion);

            break;

        default:
            fprintf(stderr, "%s: Dirección '%s'. Tipo no soportado.", fichero, p_dir->texto);
            exit(1);
    }
}
...
/**
 * Inserta una referencia a una direccion en la tabla de símbolos
 *
 * @param ts Tabla de símbolos
 * @param nombre Nombre de la etiqueta referenciada
 * @param indice Indice de la entrada dónde se hace referencia a la etiqueta
 * @param offset Desplazamiento que se le debe sumar a la dirección
 * @param tipo_dir Parte de la dirección absoluta de la etiqueta que se necesita.
 */
void ts_inserta_direccion(
        tabla_simbolos_t *ts,
        char *nombre,
        int indice,
        dword offset,
    ...
    char *tipo_entrada[] = {"Rint", "BTB.PC", "BTB.dest"};

    ref_direccion_t *p_dir;
    ref_direccion_t **p_last;

    // Va hasta el final de la lista
    for (p_dir = ts->dirs, p_last = &ts->dirs;
         p_dir != NULL;
         p_last = &(p_dir->sig), p_dir = p_dir->sig) {
        if (p_dir->tipo_dir == tipo_dir && p_dir->indice == indice) {
            char mensaje[128];
            sprintf(mensaje, "%s:%d. Entrada %d del tipo '%s' ya inicializada",
                    basename(ts->fichero), linenumber, indice, tipo_entrada[indice]);
            yyerror(mensaje);
        }
    }

    *p_last = nueva_direccion(nombre, indice, offset, tipo_dir);
}
...
void ts_resuelve_referencias_locales(
    ...
    for (p_ref = ts->refs, p_last = &ts->refs;
        ...
        if (p_etiq != NULL) {
            ...
            valor = calcula_inmediato_referencia(ts->fichero, p_ref, dir_ref, dir_instruc,
                                                 mm_global->regiones[r_data].inicio);
            instruccion.imm = valor;
...
/**
 * Resuelve las direcciones locales a un objeto
 *
 * @param ts Tabla de símbolos
 * @param mm Mapa de memoria
 */
void ts_resuelve_direcciones_locales(
        tabla_simbolos_t *ts,
        mapa_memoria_t *mm
) {
    ref_direccion_t *p_dir;
    ref_direccion_t *p_next;
    ref_direccion_t **p_last;

    etiqueta_t *p_etiq;

    ref_direccion_t *p_deleted = NULL;
    ...
    for (p_dir = ts->dirs, p_last = &ts->dirs;
        ...
        p_etiq = ts_busca_etiqueta(ts, p_dir->texto);
        ...
        if (p_etiq != NULL) {
            ...
            resuelve_direccion_referencia(ts->fichero, p_dir, dir_ref);

            // Borrar la referencia de la lista
            *p_last = p_next; // la quitamos de esta lista

            p_dir->sig = p_deleted; // La añadimos a la lista de borradas
            p_deleted = p_dir;
...
void ts_simbolo_direccion(
    ...
    if (p_sim != NULL) {
        if (p_sim->direccion == direccion) {
            ...
            if (f_etiqueta) {
                sprintf(etiqueta, "%s + %04"
                                  PRId64, p_sim->texto, direccion - p_sim->direccion);
            } else {
...
void ts_resuelve_referencias_globales(
    ...
    for (p_ref = ts->refs; p_ref != NULL; p_ref = p_next) {
        ...
        if (p_sim != NULL) {
            ...
            valor = calcula_inmediato_referencia(ts->fichero, p_ref, p_sim->direccion, dir_instruc,
                                                 mm_global->regiones[r_data].inicio);
            instruccion.imm = (half) valor;
 ...
 * @param mm Mapa de memoria
 */
void ts_resuelve_direcciones_globales(
        simbolo_t *sims_globales,
        tabla_simbolos_t *ts,
        mapa_memoria_t *mm
) {
    ...
    for (p_dir = ts->dirs, p_last = &ts->dirs;
        ...
        p_sim = ts_busca_simbolo(sims_globales, p_dir->texto);

        if (p_sim != NULL) {
            // Referencia local encontrada
            dword dir_ref;

            resuelve_direccion_referencia(ts->fichero, p_dir, p_sim->direccion);
