 ...
 *
 * Descripción:
 *   Contiene las funciones para manejar el LOG
 *
 ****************************************************************************/
...
void write_log_2(char* senyal, dword pc_read, dword pc_write, char* fase_read, char* fase_write) {
    ...
    print_log("Ciclo: %u, Señal: %s; ", Ciclo, senyal);
    fprintf(f_log, "Ciclo: %u, Señal: %s; ", Ciclo, senyal);
...
void write_log_1x(char* senyal, dword pc, ubyte reg) {
    //char instr_dst[128] = "";
    char instr_fte[128] = "";
    ...
    print_log("Instrucción %s, Reg x%d<br>\n", instr_fte, reg);
    fprintf(f_log, "Instrucción %s, Reg x%d\n", instr_fte, reg);
    fclose(f_log);
}

void write_log_1f(char* senyal, dword pc, ubyte reg) {
    //char instr_dst[128] = "";
    char instr_fte[128] = "";
