...
public class SortedRegister {
    ...
    /** Classify and sort data read from Scanner sc, line by line
    ...
    /** Recorded data items > 0 are written into pw, line by line
     *  with format
     *        day  month  amount
    ...
    public int add(Scanner sc, PrintWriter err) {
        ...
        while (sc.hasNext()) {
            try {
                ...
                err.println("ERROR. Line " + count + ": No integer data item.");
            } catch (IllegalArgumentException e) {
                err.println("ERROR. Line " + count + ": " + e.getMessage());
            }
        }
